...
fave_convert<-function(file,speaker){
library("PraatR")
library("rPython")
#####FAVE directory
...
outdf[,5]<-gsub("[[:space:]]-[[:space:]]","")
outdf<-gsub("\\(breathing\\)","{BR}",outdf,ignore.case=FALSE)
##strip quotes
outdf<-data.frame(outdf)
outname=paste(unlist(strsplit(file, split="\\."))[1],"_",speaker,"_FAVE.txt",sep="")
